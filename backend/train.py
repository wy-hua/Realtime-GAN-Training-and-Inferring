...
def test(args, genA2B, genB2A, testA_loader, testB_loader, name, step):
    ...
    with torch.no_grad():
        ...
        # #将tensor转换成图片
        # grid_A2B=utils.make_grid(A2B,normalize=True,range=(-1, 1), nrow=16)
        # ndarr_A2B = grid_A2B.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
        # im_A2B = Image.fromarray(ndarr_A2B)
        
        # grid_B2A=utils.make_grid(A2B,normalize=True,range=(-1, 1), nrow=16)
        # ndarr_B2A = grid_B2A.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
        # im_A2B = Image.fromarray(ndarr_B2A)
...
def train(args, trainA_loader, trainB_loader, testA_loader, testB_loader, G_A2B, G_B2A, D_A, D_B, G_optim, D_optim, device):
    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            ...
            msg=[i,D_adv_loss_val,lpips_val,G_adv_loss_val,G_cycle_loss_val,G_con_loss_val,G_style_loss.item()]
            with open('measure.txt', 'a+') as f:
